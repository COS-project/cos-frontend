...
interface Props {
  ...
  setIsGoalScoreAlertModalOpen: React.Dispatch<React.SetStateAction<boolean>>;
}

const GoalScoreAlertModal = (props: Props) => {
  const { maxScore, setIsGoalScoreAlertModalOpen } = props;
  return (
    <div
      className={
      ...
      <div
        onClick={() => {
          setIsGoalScoreAlertModalOpen(false);
        }}
      ...
      <div className={'flex flex-col gap-y-4 bg-white rounded-[32px] p-5'}>
        <div className={'flex flex-col gap-y-1'}>
          <div className={'text-h2 font-semibold text-black'}>목표 점수를 확인해주세요!</div>
          <div className={'flex flex-col gap-y-1'}>
            <div className={'flex items-center text-h6'}>{'목표 점수는 최소 점수 0점 이상'}</div>
            <div className={'flex items-center text-h6'}>{` 최대 점수 ${maxScore} 이하로 설정이 가능합니다.`}</div>
          </div>
        </div>
      </div>
    </div>
...
export default GoalScoreAlertModal;
